...
def synthesize_with_sbv2(
    ...
    speaker_int = int(speaker_id if speaker_id is not None and str(speaker_id).isdigit() else generation_config.get("speaker_id", 0))
    ...
    holder = TTSModelHolder(model_assets_dir, device)
    if not holder.models_info:
        return GenerationResult("failed", output_path, notes=f"no SBV2 models found in {model_assets_dir}")
    if model_name is None:
        ...
        if not matches:
            return GenerationResult("failed", output_path, notes=f"SBV2 model not found: {model_name}")
        model_info = matches[0]
    if model_file is None:
        files = [f for f in model_info.files if (f.endswith(".safetensors") or f.endswith(".onnx")) and not f.startswith(".")]
        if not files:
            return GenerationResult("failed", output_path, notes=f"no model file found for {model_name}")
        model_file = files[0]
    ...
    except Exception as exc:  # synthesis boundary: capture per-row failures for long runs
        return GenerationResult("failed", output_path, notes=f"SBV2 synthesis failed: {exc}")
